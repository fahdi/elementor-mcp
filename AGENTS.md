...
Single source of truth for every coding agent working in this repository (Claude Code, Codex, Cursor, Copilot, and anything else that reads `AGENTS.md`). `CLAUDE.md` only imports this file; do not add knowledge there. When you learn something durable about this codebase, put it here, in the section it belongs to, so the next agent (of any vendor) sees it.

## How to use this file

- Read **Working rules** first; they override defaults regardless of which agent you are.
- The **Project Overview** and release notes below are the running history of what shipped and what was learned; each release paragraph lists the traps that cost time.
- Per-feature handoff documents live next to this file when a piece of work is large enough to need its own evaluation notes: currently [WIDGET-BUILDER-DATA-LAYER.md](WIDGET-BUILDER-DATA-LAYER.md) (Widget Builder query / shortcode / remote data controls, September 2026).
- Agent-facing skills for driving the plugin's own MCP tools live in `pro/skills/*/SKILL.md`; repository skills for plugin development and review live in `.claude/skills/` (see the end of this file).

## Working rules (apply to every agent)

- **Copy style:** never use em dashes in any copy you write for this project (changelog, readme, docs, commit messages, social posts, replies). Restructure with a colon, comma, or full stop.
- **Release discipline:** build locally and stop. Never push, tag, create a GitHub release, upload to Freemius, or deploy until the user explicitly says "push" or "release". Commit when asked.
- **Design:** flat colours only, no gradients, no generic AI-looking layouts (website, admin, widgets, social banners).
...
**3.17.1 (local, built 2026-09-21, unreleased).** Patch batch from the open-issue review: (1) **#145** `EMCP_Tools_Data::save_page_settings()` now merges the patch into the stored `_elementor_page_settings` BEFORE `$document->save()`; Elementor's page-settings manager is a full replace, and a one-key patch on the kit post wiped the whole kit. `update-global-colors` refuses `EMCP_Tools_Global_Abilities::SYSTEM_COLOR_IDS` (`reserved_color_id`) and returns `target`/`written`; system colors stay with the Pro `replace-system-colors`. (2) **#146** `EMCP_Tools_Element_Factory::normalize_dimension_settings()` / `normalize_dimension_tree()` cast numeric `top/right/bottom/left/column/row` to strings on every `save_page_data()`; the editor's Layout panel shows 0 for a numeric side while the CSS renders fine. Slider `size` stays numeric, `$$type` props are skipped. (3) **#148** `EMCP_Tools_Cloud_Connect::pending_record()` captures `was_provisioned`; `gateway_action()` returns provision / deprovision / none (ticked box re-issues, unticked on a provisioned site withdraws); `reissue_gateway()` + `ACTION_REISSUE` back a "Re-issue gateway credential" button on the Connection tab. (4) **PR #149** (fernandotellado, issue #141): free `EMCP_Tools_Visibility_Integration` (`includes/abilities/seo/`) over Visibility 2.7.1's per-key `_native_aeo_pack_*` post meta; the PR shipped only the class + registrar + require, the rest was wired here: `wp_visibility` catalog, `visibility_available()`, `seo_tool_slugs()` (16), `DEFAULTS_VERSION` 52 to 53 seeding `visibility-write` off, test class map, `VisibilityIntegrationTest`, SEO skill. `schema_type` now refuses invalid values (`invalid_schema_type`) and accepts `''` to clear, matching Visibility's own sanitizer. Live smoke on msrplugins.test passed all 21 checks (Visibility installed there). Suites: pro 2908, public 232. (5) **#143 embedded bug, fixed:** `save_page_data()` now compares `EMCP_Tools_Data::collect_element_ids()` of what it sent against what the native save persisted and forces the direct meta write when any id vanished. Root cause read from Elementor source: `get_elements_raw_data()` does `create_element_instance()` and `continue`s on null, so a widget type not registered in the CLI/REST context is dropped and `save()` still returns true. Reproduced live on Elementor 4.2.4 with a real CartFlows `checkout-form` (registers only when `$post` is a `cartflows_step`); FunnelKit free 3.16.0.5 already registers `wfacp_form` on every request as its own workaround, so the reporter's loss was most likely a Pro-only or CartFlows widget. WooCommerce, FunnelKit free and CartFlows are now installed on msrplugins.test for this. Guard: `pro/tests/unit/regression/UnregisteredWidgetPreservationTest.php`. (6) **OAuth discovery scope (support email, dev.mybcn.es, EMCP 3.16.1 + Tropk MCP for WordPress 0.5.5):** `EMCP_Tools_OAuth_Metadata::path_matches()` prefix-matched ANY suffix under both well-known paths, so EMCP answered Tropk's RFC 8414 request `/.well-known/oauth-authorization-server/wp-json/tropk-mcp/v1/` with its own document (issuer mismatch, client stops before consent). Now a suffix must be one of `own_resource_paths()` (the path of `resource()`, raw and home-stripped for subdirectory installs); anything else falls through. Verified live over HTTPS on msrplugins.test: EMCP paths 200 with EMCP JSON, Tropk-scoped paths fall through (404 there, Tropk's own handler on the reporter's site). Note EMCP already advertises its own REST `resource_metadata` URL in `WWW-Authenticate` for coexistence; the well-known handler is only for clients deriving the path themselves. Not done, still open: #143 `funnelkit-write` itself and #144 FunnelKit Automations.

**3.17.0 released (2026-09-21).** GitHub tag `v3.17.0` points to `3c6b78d`; the Free ZIP is attached to the public release. Pro deployment `154381` is released on Freemius with `is_released=false` preserved, matching prior deployments. The release supersedes 3.16.1. Consolidates the standalone builder integrations, independent Gutenberg packs, Widget Builder data controls and reserved-control fixes, bundled skills and Templates module visibility fix. `emcp-tools.php`, `readme.txt` and `CHANGELOG.md` target 3.17.0. The Templates library now follows its module state independently of selected builder; its contents remain Elementor-specific. `CLAUDE.md` continues to import this file. Release checks and remaining publication steps: [docs/RELEASE-3.17.0.md](docs/RELEASE-3.17.0.md). Runtime skills and shell scripts use LF via `.gitattributes`; the build verifier checks the new Pro adapters and bundled skills explicitly.

**3.17.0 website release materials.** The sibling website has 15 new builder/block-pack landing pages and references, a compatibility guide, separated theme pages and a release article. Publication record: `website/docs/RELEASE-3.17.0.md`. Its tool-count generator must count the full catalog, not `get_available_tool_slugs()` (filtered by selected builder); fresh-install defaults are measured with request-local filters, never by changing saved site options. New integration pages may omit competitor comparisons when no applicable research exists.

**Otter Blocks integration (local, unreleased).** Independent Gutenberg pack `otter`, 19 native tools, defaults migration 52, and injected/downloadable `emcp-otter` skill. Verified Free/Pro 3.2.5: 45 registered blocks without Pro, 63 with Pro. Block packs with `native_tools` use their own adapter instead of the legacy dispatcher wrapper; keep this distinction in registration, admin catalog and defaults migration. Static Otter content needs native saved HTML plus attributes; writes regenerate CSS through `ThemeIsle\GutenbergBlocks\CSS\CSS_Handler::generate_css_file()`. Atomic Wind utility CSS is a separate browser compiler. Test page 2242 was created over MCP and saved successfully in Gutenberg, including Pro Business Hours and native Accordion. Scope, fixtures and results: [docs/OTTER-INTEGRATION.md](docs/OTTER-INTEGRATION.md).

**Visual Composer integration (local, unreleased).** Standalone Website Builder 45.16.2 adapter with 16 tools, defaults migration 51, and an injected/downloadable `emcp-visual-composer` skill. The native editor compiles HTML/CSS in JavaScript: stage complete paired element and render artifacts, then explicitly publish. `vcv-pageContent` alone does not change the frontend. Page CSS must reach `vcvSourceCss` and the native Assets helper must generate its file. Global options remain read-only; optional Theme Builder add-ons were absent. Native editor round trip and 320 px preview verified on test page 2238. Scope and results: [docs/VISUAL-COMPOSER-INTEGRATION.md](docs/VISUAL-COMPOSER-INTEGRATION.md).

**Blocksy separation (local, unreleased).** Blocksy Blocks is an independent Gutenberg pack (`blocksy-blocks`) with eight individual tools and two compatibility dispatchers. Theme layout/design tools stay in Themes; Companion extensions and Pro Content Blocks live in Plugins. Sixteen tools total, defaults migration 50. New adapters must stay in the Pro loader, manifest and test class map. Native 2.1.57 findings: Content Blocks use `ct_content_block`, `template_type` and `blocksy_post_meta_options`; create disabled drafts and publish explicitly. The native `content-block` embed reads `content_block` even though that attribute is absent from its server registry. Theme writes must call `blocksy_manager()->db->wipe_cache()` before CSS refresh or the same request sees stale values. Native extension toggles need the manager's `can()` gate. Details, scope and live smoke commands: [docs/BLOCKSY-INTEGRATION.md](docs/BLOCKSY-INTEGRATION.md).

**Beaver Builder integration (local, unreleased).** Standalone Pro adapter with 22 tools, native draft staging and explicit publication; implementation and live-test results are in [docs/BEAVER-INTEGRATION.md](docs/BEAVER-INTEGRATION.md). Verified against Pro 2.11.1 with 15 authorable core module types and 42 discoverable module schemas. Native `save_layout(false)` already generates assets: rendering them a second time in the same request can silently omit module/page CSS. Native access defaults can cache before WP-CLI user authentication; refresh the existing builder-access registration without changing saved roles. Native layout caches can contain slashed settings, so publication reads persisted unslashed metadata. Missing photo sources require attachment URL resolution. Keep the helper in the Pro loader before the adapter and both classes in the test class map. Optional Themer and shared/global/dynamic references remain read-only.
...
**Widget Builder data layer (2026-09-18 to 2026-09-20, committed locally, unreleased).** The Sandbox Widget Builder DSL gained three data controls: `query` (sources `posts`, `products`, `terms`, `menu`, `site`, `breadcrumbs`, `cart`, `remote`), `shortcode` (allowlisted providers: saved Elementor templates and the major form plugins) and the `remote` query source (OpenWeather, Google Reviews, Yelp, generic JSON presets; keys on the Connection tab under "Live data providers", `EMCP_Tools_Remote_Keys`). Runtime classes `EMCP_Tools_Widget_Query`, `EMCP_Tools_Shortcode_Registry`, `EMCP_Tools_Remote_Source` sit in `pro/includes/` and MUST stay in `EMCP_Tools_Pro_Loader::FILES` (runtime), not `MCP_FILES` (deferred surface skipped on front-end views). The generator now emits structured defaults for `icon`/`url`/`media`/multi-`select2`. 100 agent-built widgets exist on the local site `elementor-widgets.test` as the live reference (showcase pages 132, 155, 195, 226, 250, 276, 316; per-widget documentation pages with all layouts, a setup guide and an options reference at `/widgets/`). Full handoff, contracts, and traps: [WIDGET-BUILDER-DATA-LAYER.md](WIDGET-BUILDER-DATA-LAYER.md). **2026-09-21 (uncommitted, in `pro/`):** `EMCP_Tools_Widget_Generator::RESERVED_CONTROL_NAMES` makes `validate_spec()` refuse a control named `animation` / `_animation` (Elementor reads that setting on every widget as its entrance animation and adds `elementor-invisible`, so the widget renders hidden; two shipped specs had it), guarded by `test_validate_rejects_elementor_reserved_control_names`; the widget-builder skill gained that rule plus the "filled active states need their own colour control" rule. Three spec-level traps found while auditing the doc pages, all fixed on the test site and written up in the handoff doc: reserved `animation` name, control defaults overriding filled-state CSS (pill tabs, inline chips, dark table header), and JS that reparents a fixed element to `<body>` (loses the element-scoped control CSS).
...
**Current status: v3.14.2 patch release preparation. MCP startup serialization, canonical public URLs, OAuth resource binding and discovery diagnostics, complete Application Password handshake testing, and the bundled MCP Adapter 0.6.1.**

1. **BeTheme + BeBuilder integration (Pro).** `EMCP_Tools_BeTheme_Integration` (`pro/includes/abilities/class-betheme-integration.php`) extends `EMCP_Tools_Theme_Integration` on the two-dispatcher pattern: `betheme-read` / `betheme-write`, 8 operations (`get-context`, `get-settings`, `list-item-types`, `get-item-schema`, `get-page`, `update-settings`, `set-page`, `add-section`). Registers only when `get_template()` is BeTheme. **Two traps that cost the most time, both in `pro/skills/emcp-betheme/SKILL.md`:** (a) a BeBuilder item's settings live under **`attr`**, not `fields`; (b) a wrap or item without a **`size`** renders as **nothing at all**, silently. Theme Options are a 34-key `ALLOWLIST` over the single autoloaded `betheme` option (830 keys); `item_schema()` reads the real field contract by reflecting each `sc_*` shortcode's `shortcode_atts` block. `DEFAULTS_VERSION = 35` seeds `betheme-write` disabled. Free-tree `EMCP_Tools_Themer_BeTheme_Conflict` (`includes/themer/class-themer-betheme-conflict.php`) warns (never auto-resolves) when a Themer body template and BeTheme's own template system would both claim a page. Tests: `pro/tests/unit/themes/BeThemeIntegrationTest.php` + `pro/tests/unit/Themer/BeThemeConflictTest.php`.

2. **Malware-scanner false positive on the migration upload paths, plus real hardening.** A customer's host flagged `class-restore-engine.php` as a PHP backdoor. It was a false positive: the chunked upload accepts a base64 archive as a WAF fallback, and "decode request input, write it to a file" is the dropper shape, indistinguishable by shape from the real thing. **Decoding now runs through a `convert.base64-decode` stream filter** in `EMCP_Tools_Restore_Engine::write_encoded_chunk()` and `EMCP_Connector::decode_into()`, so the archive bytes never exist as a PHP string. The connector hashes the packet **as a stream** for the signature, which needed `packet_canonical_hashed()` on both sides (canonical bytes unchanged, so an already-paired 1.2.0 connector still works). Two real gaps closed: the upload refused **nothing** (now shares `UPLOAD_EXTENSIONS` with the listing), and the connector's media sync wrote **any** filename (now refuses executables across every extension in the name, dotfiles, and typeless names, with the no-filename fallback typed from the declared mime via `ext_for_mime()`). Connector **1.3.0**. **Never leave a `base64_decode` literal near an `fwrite` in shipped code, comments included** (same family as the #100 lesson).

3. **Admin.** Tool cards that cannot be switched on carry a "Needs X" badge naming the missing plugin/theme (`requirement_badge()` / `requirement_note()`, 53 notes converted to `requires => { name, kind }`). The settings-saved notice renders **server-side as the finished toast** (`.emcp-toasts > .emcp-toast`), bottom right; `admin.js` only wires dismissal. It must keep the `inline` class or core's `common.js` relocates it back to the top on jQuery ready.

**Prior status: v3.13.0. The Themer widget library + dynamic data release, with a rewritten SQL guard.**

**1. Themer dynamic data (free tree, with a Pro overlay).** A field binds to a *source* instead of needing a bespoke widget. One **value-first** provider (`EMCP_Tools_Themer_Dynamic::value( $key, $args ): array{type,value}`) feeds five surfaces: element render, Elementor dynamic tags, WordPress block bindings, the MCP write tools, and the editor preview. `EMCP_Tools_Themer_Dynamic_Catalog` (`includes/themer/dynamic/`) is the single source list; each entry declares `label, icon, type, args, bindable, element`. Surfaces: `EMCP_Tools_Themer_Elementor_Tags` + `_Tag_Classes` (one parameterised Tag class; `categories_for()` returns `array()` for `html` sources so markup sources never register as tags), `EMCP_Tools_Themer_Block_Bindings` (`SOURCE = 'emcp/themer'`, `field_accepts()` type matrix), `EMCP_Tools_Themer_Dynamic_Compiler` (`for_elementor()` writes the `__dynamic__` key + `[elementor-tag …]` shortcode, `for_blocks()` writes `metadata.bindings`; both refuse an unknown source and a type mismatch), and `EMCP_Tools_Themer_Dynamic_Abilities` (`list-dynamic-sources`). **This works on FREE Elementor**: Elementor ships the dynamic-tag machinery but no sources, so the picker is empty until we fill it. 10 free bindable sources; **7 Pro** (`custom-field` ACF-aware, `author-{name,bio,url,avatar}`, `term-{name,url}`) plus an empty-value fallback and date/text formatting, via `EMCP_Tools_Themer_Dynamic_Pro` on the `emcp_themer_dynamic_sources` / `emcp_themer_dynamic_value` filters. In the editor there is no queried post, so `preview_post_id()` samples one; the sample is never persisted.

**2. Themer widget library.** **19 Elementor widgets / 14 Gutenberg blocks / 22 free catalog sources.** Nine widgets are new (Author Box, Post Navigation, Post Comments, Search Form, Sitemap, Post Info, Archive Posts, Featured Image, Post Excerpt); Author Box + Sitemap also ship as blocks (comments/search/post-navigation are core blocks already, deliberately not duplicated). Markup lives in builder-agnostic element classes (`includes/themer/elements/class-themer-element-*.php` on a shared `_Base` giving `post_id()`/`truthy()`/`wrap()`), so a widget and its block cannot diverge. Widgets with bespoke controls extend `EMCP_Tools_Themer_Widget_Rich_Base` (`includes/themer/widgets/rich/`) and delegate render to the same dispatcher. **`post-meta` and `archive-loop` are deliberately untouched** (existing templates must render unchanged); `post-info`/`archive-posts` are successors beside them, and `archive-loop` is relabelled "Archive Posts (classic)". `Archive_Posts` reads the **main query without iterating it** (a Themer body is still using that query).

**3. Widget-category promotion.** While an `emcp_theme_template` is the open document, the EMCP Themer category is moved to just under Favorites. Elementor offers **no supported seam**: `add_category()` only appends (Elementor's own source says so), `$categories` and `promote_category_after()` are `private`, and `elementor/document/config` runs through `array_replace_recursive()` which preserves key order. So it is done by **reflection**, wrapped to fail into the old order. The widgets themselves stay registered everywhere on purpose.

**3b. The 9 Themer tools finally reached the Tools grid.** They had registered since 3.1.0 with **no admin-catalog entry**, so an admin could not see or toggle them and `get_available_tool_slugs()` (and therefore every count the plugin and website report) omitted them: 251 to **260**, free 193 to **202**. New `themer` category on the **wordpress** platform tab, module-gated like `themer_php`, plus `EMCP_Tools_Admin::themer_tool_slugs()` for the drift-guard exclusion. All 9 stay enabled by default; the Modules toggle stays the single kill switch. Tests: `pro/tests/unit/admin/ThemerToolCatalogTest.php`.

**4. Read-only SQL guard rewritten on a real tokenizer** (`includes/sql/class-sql-lexer.php` + `class-sql-policy.php`; `EMCP_Tools_Database_Guard` shrank 286 to 69 lines and delegates). Fail-closed: it accounts for every byte or refuses. Analyses under all 4 `ANSI_QUOTES` x `NO_BACKSLASH_ESCAPES` readings and allows only if safe under every one (`MODE_DEPENDENT_ERRORS` skips readings that are syntax errors in their own mode). Refuses `/*!` and `/*+`, unterminated constructs, non-UTF-8, and the server system schemas. Empirical rule worth keeping: **NUL is not treated as a comment introducer** even though MySQL comments on other C0 controls, because being a strict SUBSET of the server's comment set is the safe direction.

**5. OAuth loopback fix.** A CLI app registers `http://localhost:PORT` and returns on `http://127.0.0.1:PORT`. `EMCP_Tools_OAuth_Util::is_loopback_host()` now treats `localhost`/`127.0.0.1`/`::1` as one machine (RFC 8252 §7.3), trailing slash ignored. Non-loopback redirects still match exactly.

**Four traps this release caught, all silent-failure shaped:**
- **`emcp_pro_fs()` is defined NOWHERE.** Six files called it. Five wrote `function_exists(...) && ! ...`, which fails OPEN, so their licence gate was inert. One wrote `! function_exists(...) || ...`, which fails CLOSED, so all 7 Pro dynamic sources never registered on any site. **The accessor is `emcp_tools_fs()`.** Guard: `pro/tests/unit/Bootstrap/ProLicenseAccessorTest.php`.
- **`pro-manifest.txt` and the Pro loader's file list are two different lists.** `class-themer-dynamic-pro.php` was in the manifest (so it shipped) but not in `EMCP_Tools_Pro_Loader::FILES`, so it never loaded, and the `class_exists` guard at its call site made that silent. Same guard test pins both directions.
- **`printf "$listing" | grep -q` under `set -o pipefail` inverts.** grep -q exits on first match, printf takes SIGPIPE, pipefail calls the pipeline failed. In `tools/verify-release-zip.sh` this made the **Pro-leak gate report "no leak"**. Plus `pro-manifest.txt` is CRLF, so `read -r p` left a `\r` on every entry and matched nothing: the gate was a no-op on every Windows checkout. Fixed with here-strings + CR strip + a canary that hard-fails if the matcher stops finding something that IS in the zip.
- **A Themer element renders against the QUERIED object, not `$GLOBALS['post']`.** Setting the global alone in a test or smoke gives blank output that looks like a bug and is not.

Tests: `pro/tests/unit/Themer/` (Themer suite) + `pro/tests/smoke/themer-{widget-library,category-promotion}-smoke.php` (live `wp eval-file` harnesses). Suites: pro **2346**, public **155**.

**Prior status: v3.7.0 (tagged) — the Sandbox Blocks + Agent Project Memory release.** Two Pro features: (1) **Gutenberg Block authoring in the Sandbox** — the Sandbox parent page is now a 3-card overview (Blocks / Widgets / PHP Snippets), each with its own `?page=emcp-tools-widgets&view=…` management screen. An AI agent designs a custom Gutenberg block from a structured spec; the plugin compiles it into a **sandboxed dynamic block** (block.json + PHP render) under `wp-content/emcp-sandbox` via a shared `Sandbox_Template_Compiler`, draft-by-default, activated by a human, appearing in the inserter under "EMCP Custom" with a server-side live preview. `EMCP_Tools_Block_Store` + block loader; blocks/widgets export/import as portable JSON bundles (MCP + admin, imports always land as a new inactive draft) — the **cloud-ready artifact backbone** for a future EMCP Cloud sync. New `emcp-blocks` skill. **DEFAULTS_VERSION=24** for the sandbox work. **Two live-caught traps:** a `file:`-registered render omits `editorScript`/`style` and the loader registers them with `Sandbox_Store::artifact_url()` + wp-* deps set directly on the `WP_Block_Type` (block.json `file:` URLs mangle for uploads-dir blocks); and `render.php` must **echo, not return** (WP wraps it in its own `ob_start`/`require`/`ob_get_clean` and ignores the return). (2) **Agent Project Memory** — the plugin remembers a site across agent sessions so a connected agent stops guessing (the AUTO-populated companion to the free **Context** tab). CPT `emcp_memory` where **post status is the trust gate** (`pending` agent-proposed → `publish` admin-approved → injected); approved guidance composes a "## Project memory" block injected into discovery context via a new free `emcp_tools_discovery_memory` seam (mirrors the skills seam). 3 Pro MCP tools (`recall`/`remember`/`save-session-summary`, disabled-by-default), a capped `emcp_tools_memory_sessions` option, a change-ledger-derived `EMCP_Tools_Memory_Digest`, an hourly rollup + opt-in AI-Chat LLM summary, and a new **Memory** admin tab (approve/reject/edit/settings). Behind a **Memory** module kill-switch (on by default). **DEFAULTS_VERSION=25.** Deferred fast-follows (data models ready): guardrail ENFORCEMENT (a `Memory_Enforcer` on `severity=block` + structured `target`) and cross-site memory sync over the sandbox cloud-bundle contract. Classes in `pro/includes/memory/`; tests `pro/tests/unit/memory/` (Memory suite).

**Backup/Migrate/Sync Pro module — CODE-COMPLETE (Phases 1–4), unreleased/local-only. Phase 1 Backup+Restore = v3.12.x; Phase 2 Connector+Migration = v3.13.0; Phase 3 Sync (full+selective) = v3.14.0; Phase 4 MCP tools = v3.15.0.**

**Phase 4 (v3.15.0, BUILT + live-verified) — MCP tools.** `EMCP_Tools_Migrate_Abilities` (`pro/includes/abilities/class-migrate-abilities.php`) exposes 4 tools, reusing the engines: **`create-backup`** (drives `Backup_Engine` to completion via a `run_chunk_sync` loop; returns id/size/download URL) + **`list-backups`** — non-destructive, **on by default**; **`migrate-site`** (`push()`+`trigger_restore()`; optional `backup_id` else builds a fresh full backup) + **`sync-to-live`** (`Sync_Engine::build_scoped_emcp`→push→restore) — **destructive, disabled-by-default, `confirm:true` required** (confirm-gate short-circuits before any engine work). Self-gates on premium; executors `ensure_engines()` lazy-load the migrate classes (MCP call may precede the module's init:5 boot). Loaded via `EMCP_Tools_Pro_Loader` file list; registered by the registrar gated on `EMCP_Tools_Migrate_Module::is_enabled()`. Admin: `EMCP_Tools_Admin::migrate_tool_slugs()` (drift-guard exclusion — module-gated), **`DEFAULTS_VERSION` 32→33** seeds `migrate-site`+`sync-to-live` disabled, `migrate` catalog category ("Backup & Migrate", platform wordpress). `pro-manifest.txt` += `includes/abilities/class-migrate-abilities.php`. Tests `MigrateAbilitiesTest` (Migrate suite **69**). **Live-verified:** all 4 register when the module is on, the destructive pair is disabled-by-default, `migrate-site` without `confirm` → `WP_Error(confirm_required)`, `list-backups` executes. No delete-backup tool (deletion stays admin-only).

**Phase 1 (Backup+Restore) = v3.12.x; Phase 2 (Connector + local→live Migration) = v3.13.0.**

**Phase 2 (v3.13.0, BUILT + live-validated) — connector + migration.** A **Migrate** sub-tab pushes this site to a live server without a manual upload. The source (Pro) streams the `.emcp` to a standalone, **unlicensed connector plugin** (`pro/connector/emcp-connector.php`, served as a ZIP by `EMCP_Tools_Connector_Builder`, zero EMCP-class deps) at REST `emcp-connector/v1` (`pair`/`receive-packet`/`restore`/`status`) in **HMAC-signed, base64-encoded packets with offset-based resume** (`EMCP_Tools_Migrate_HMAC` canonical strings are re-implemented byte-identically in the connector — parity unit-tested; base64 so the raw SQL/PHP clears a destination WAF, the same lesson as the browser upload). `EMCP_Tools_Migrate_Targets` stores paired sites (`{prefix}emcp_migrate_targets`, encrypted secret, single-use HTTPS pairing code; **DB_VERSION 3**). `EMCP_Tools_Migration_Engine` = pure `packet_size()`/`plan_packets()`/`upload_id()` + `push()` (resumes on the connector's `next_offset` 409) + `trigger_restore()`/`poll_status()`. The connector restore is **self-contained** — transcribed `.emcp` extractor + DB import (**reusing the Phase-1 transaction-directive skip**) + serialization-safe search-replace (**reusing byte-accurate `fix_serialized_strings` + raw-JSON**) + file placement — runs past client disconnect (`ignore_user_abort`), never touches `wp-config.php`/salts, progress in an option the source polls. Admin AJAX (pair/list/delete-target, push, push_progress, remote_status) on the module. **Build: the connector ships as a top-level `connector/` dir in the PRO zip only** (`pro/tools/build-release.sh` copies `pro/connector`; `pro-manifest.txt` lists `connector`; free git-archive never includes it). **Live-validated** (connector restore on emcp-bckup-test.test): plain + serialized URLs rewritten (byte-accurate `s:LEN`), files placed into ABSPATH, salts md5 untouched. **Windows trap fixed live:** `place_files` must `wp_normalize_path()` both the work-root and `RecursiveDirectoryIterator::getPathname()` or the prefix-strip fails on backslash paths. Tests: `pro/tests/unit/migrate/` Hmac/Targets/PacketMath/ConnectorAuth/ConnectorRestore (Migrate suite 58). **Deferred:** the source-side push is a single (long, `set_time_limit(0)`) admin-ajax request — resumable but not yet chunked across requests; full HTTP cross-site (pairing + network push) is the user's end-to-end test; Action Scheduler still deferred. **Phases 3 (sync: full+selective) & 4 (MCP tools `create-backup`/`migrate-site`/`sync-to-live`) not built.**

**Phase 3 (v3.14.0, BUILT + live-validated) — one-way Sync (full + selective).** A **Sync** sub-tab pushes to a paired target as **Full** (`{db:all,files:all}`) or **Selective** (chosen tables and/or file roots). `EMCP_Tools_Sync_Engine` builds a **scoped `.emcp`** — `normalize_scope()` (`{db,files}`, defaults→all, empty→none) + traversal-guarded `file_root_path()` (themes/plugins/uploads + safe relative pass-through), `build_scoped_emcp()` (chosen tables via `DB_Exporter::export($tables)`, chosen roots via a bounded `RecursiveIteratorIterator` walk into `Packager::add_file`, `is_path_excluded` drops caches/backups/connector dirs) — carrying `scope` in the manifest, transferred over the **unchanged Phase-2 packet path**. The connector applies **only** the archived tables/files (**partial restore, natural**) and its `run_restore` now **gates search-replace on `$did_db`** so a files-only sync never rewrites the live DB. Module AJAX `sync_meta`/`sync_start`; `sync.php` panel + `initSync`. Reuses `push()`/`trigger_restore()`/`poll_status()`. **Live-validated** (connector-direct, two scoped archives): DB-only scoped restore rewrote its table's URL and placed **no** files; files-only scoped restore placed the file and left a DB sentinel **unchanged** (SR skipped). **Test-measurement gotcha:** a DB-scoped restore's SR legitimately rewrites `wp_options` (incl. a sentinel), so isolate the files-only SR-skip assertion with a sentinel set *immediately before* it. Tests: `SyncScopeTest` (Migrate suite 65). Phase 4 (MCP tools) remains.

**Phase 1 (Backup + Restore).** A Pro module `EMCP_Tools_Migrate_Module` ("Backup, Sync, and Migrate", on by default, Modules-tab kill-switch) that packages a whole site into a portable single-file `.emcp` archive and restores it — the plugin-side foundation for local↔live sync and local→live migration via a thin connector (Phases 2–4, own plans; cloud usage later). Engine ported from the standalone `5dp-backup-restore` plugin (renamed `EMCP_Tools_*`, `.emcp` ext, `{prefix}emcp_migrate_backups`/`_jobs` tables, protected `wp-content/emcp-backups/`): `EMCP_Tools_Packager` (streamed `.emcp` MAGIC/manifest/footer), `_Environment` (adaptive chunking + method detection), `_Job_Manager`, `_Background_Processor` (3-tier: Action Scheduler → WP-Cron loopback → AJAX; **Action Scheduler bundling deferred to Phase 2 — it degrades cleanly and 5dp shipped without it**), `_DB_Exporter`/`_DB_Importer` (mysqldump→PDO→mysqli export; chunked statement-by-statement `$wpdb` import), `_File_Archiver`/`_File_Extractor`, `_Backup_Manifest`, `_Search_Replace`, and the `_Backup_Engine`/`_Restore_Engine` orchestrators. Classes in `pro/includes/migrate/`; admin tab `page-migrate.php` + `migrate/{backup,restore,history}.php` panels + `migrate.js`/`.css` (flat); MCP tools land in a later Phase. Tests `pro/tests/unit/migrate/` (Migrate suite, 30). **Live-validated end-to-end** (full DB backup on msrplugins.test → restore onto a clean WP 7.0.3 at emcp-bckup-test.test): site URLs + Elementor `_elementor_data` rewritten and persisted, 0 serialized corruption, wp-config salts untouched. **The live smoke caught three real robustness bugs in the ported search-replace/import — read before touching this domain:** (1) serialized search-replace fataled on `__PHP_Incomplete_Class` / corrupted serialized objects whose source-site class is absent on the target — replaced with a **byte-accurate `fix_serialized_strings()`** that rewrites `s:LEN:"…"` payloads + fixes lengths and never instantiates a class; (2) the chunked importer ran the dump's `SET AUTOCOMMIT=0; START TRANSACTION; … COMMIT;` wrapper on the shared `$wpdb` connection and left autocommit **off**, so every post-import write (search-replace) was visible in-connection but **silently rolled back** at request end — the importer now **skips session transaction-control directives** (`EMCP_Tools_DB_Importer::is_transaction_control()`); (3) the JSON branch decoded before replacing, unescaping `https:\/\/host` and defeating the JSON-escaped pair so **Elementor data URLs never rewrote** — JSON is now a **raw `str_replace`** (no length prefixes) driven by the escaped+unescaped pair variants. Email-domain occurrences (`user@host`) and third-party API caches are correctly left untouched by URL search-replace.

**Prior status: v3.6.0 — the Elementor Addons release.** Adds the **Elementor Addons domain** (Pro, 3 plugins, 4 tools) on a new **Elementor Addons** group under the Plugins Tools sub-tab, plus free Themer/UAE conflict handling and the #100 resilience fix.
...
- WordPress >= 6.9 (the Abilities API — `wp_register_ability()` — is core in 6.9+)
...
- WordPress Abilities API — core in WP 6.9+
...
**Release article links are mandatory.** When publishing a release blog post, add its version-to-URL mapping to the website `src/data/release-blogs.json`. Verify the homepage Latest updates card and the full changelog both show the article link. Publishing the blog alone does not add these links.
...
- `EMCP_Tools_PHP_Snippet_Store` (`includes/class-php-snippet-store.php`) — `emcp_php_snippet` CPT + sandbox (`wp-content/emcp-sandbox/snippets/{id}.php`, `.htaccess`-blocked). Drafts have **no executable file**; activation wraps the code in `emcp_php_snippet_{id}()`, writes it, records a sha256 manifest entry.
...
Two repository skills are configured in `.claude/skills/`; agents that support skill folders can load them, others can read the `SKILL.md` files directly:
