...
$backup='emcp_beaver_test_disabled_backup';
if (($args[0]??'')==='restore') {
    $saved=get_option($backup,null);
    if ($saved!==null) {
        $is_beaver=static fn($name)=>str_starts_with($name,'emcp-tools/beaver-');
        $current=get_option('emcp_tools_disabled_tools',array());
        update_option('emcp_tools_disabled_tools',array_values(array_unique(array_merge(array_filter($current,static fn($name)=>!$is_beaver($name)),array_filter($saved,$is_beaver)))));
    ...
    if(($args[1]??'')==='new-integration-defaults') {
        ...
        foreach(EMCP_Tools_Beaver_Integration::definitions() as $slug=>$definition) { if($definition[3]) { $disabled[]='emcp-tools/beaver-'.$slug; } }
...
$disabled=get_option('emcp_tools_disabled_tools',array());
$names=(new EMCP_Tools_Beaver_Integration())->get_ability_names();
update_option('emcp_tools_disabled_tools',array_values(array_diff($disabled,$names)));
WP_CLI::success('Enabled Beaver tools for the local MCP test.');
